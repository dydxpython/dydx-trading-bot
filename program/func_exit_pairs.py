...
def manage_trade_exits(client):
    ...
    #Opening JSON file (which contains all our active trades)
    try:
    ...
    exchange_pos = client.private.get_positions(status="OPEN") #get positions that are live right now
    all_exc_pos = exchange_pos.data["positions"]
    markets_live = []
    for p in all_exc_pos:
        markets_live.append(p["market"]) #gives us a list of all markets that we have open positions for
    ...
    for position in open_positions_dict:
        ...
        #Extract position matching information from file for market 1
        position_market_m1 = position["market_1"]
        position_size_m1 = position["order_m1_size"]
        position_side_m1 = position["order_m1_side"]

        #Extract position matching information from file for market 2
        ...
        #Get order info for market 1 (m1) per exchange
        ...
        #Get order info for market 2 (m2) per exchange
        ...
        #Perform matching checks (check records from json file matches the details from the order in dydx)
        ...
        if CLOSE_AT_ZSCORE_CROSS:
            ...
            #Initialize z_scores (so we have the record of what the hedge ratio and z score was when we opened the trade)
            hedge_ratio = position["hedge_ratio"]
            z_score_traded = position["z_score"]
            if len(series_1) > 0 and len(series_2) == len(series_2):
        ...
        if is_close:
            ...
            try:
                ...
                close_order_m1 = place_market_order(client, market=position_market_m1, side=side_m1,
                                                    size=position_size_m1, price=accept_price_m1, reduce_only=True)
                ...
                close_order_m2 = place_market_order(client, market=position_market_m2, side=side_m2,
                                                    size=position_size_m2, price=accept_price_m2, reduce_only=True)
        ...
        #Keep record if items and save
            ...
            save_output.append(position) #if we didnt say close the trade it is still an open trade, therefore put it back in this empty list and save it. That empty list then becomes our new bot agents.json
