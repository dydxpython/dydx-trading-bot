...
def open_positions(client):
    ...
    try:
       ...
       bot_agents = [] #should only be an empty list if that file did not exist/ if it cannot open this file.

    #Find ZScore triggers (we need to be hunting for any of these pairs that are showing a promising Z score that will signal us to go and enter or exit a trade).
    for index, row in df.iterrows():

        #Extract our variables (take the columns from cointegrated_pairs.csv)
    ...
    # print(series_1[-1], series_2[-1])
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRESH:
                ...
                #Ensure like-for-like not already open (diversify trading). Do not opoen the same trade multiple times
                is_base_open = is_open_positions(client, base_market)
                is_quote_open = is_open_positions(client, quote_market)

                # Place trade
                if not is_base_open and not is_quote_open: #we do not have any positions with those markets in currently, no duplication of trades!
                    ...
                    #Get size
                    base_quantity = 1 / base_price * USD_PER_TRADE #make sure that we are placing the USD equivalent of that crypto
                    ...
                    #Ensure size (dydx minimum order size requirements): #alternatively also check for max order size
                    ...
                    if check_base and check_quote:
                        ...
                        free_collateral = float(account.data["account"]["freeCollateral"]) #amount available to trade
                        ...
                        #Create Bot Agent
                        #create an instance of our Bot Agent:
                        bot_agent = BotAgent(
                            client,
                        ...
                        if bot_open_dict["pair_status"] == "LIVE":
                            ...
                            del(bot_open_dict) #we dont need bot_open_dict anymore after appending it to another list (free up some memory)

                            #Confirm live status in print
                            print("Trade status is: Live")
                            print("---")
    ...
    if len(bot_agents) > 0:
        with open("bot_agents.json", "w") as f:
            json.dump(bot_agents, f) #save into a JSON file any trades that we have open so that we can then manage those existing trades
